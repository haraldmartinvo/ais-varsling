...
Kjøres periodisk (f.eks. GitHub Actions hvert 10. min). Holder tilstand i
state.json og sender e-post ved ankomst/avgang.

Miljøvariabler (settes som GitHub Secrets):
  BW_CLIENT_ID      BarentsWatch klient-ID (f.eks. epost@dom.no:mittklientnavn)
  BW_CLIENT_SECRET  BarentsWatch klienthemmelighet
  SMTP_HOST         SMTP-server (f.eks. smtp.gmail.com)
  SMTP_PORT         SMTP-port (587)
  SMTP_USER         SMTP-brukernavn (e-postadresse)
  SMTP_PASS         SMTP-passord / app-passord
  EPOST_FRA         Avsenderadresse (vanligvis = SMTP_USER)
"""
...
def send_epost(emne: str, tekst: str, mottakere: list[str]) -> None:
    host = os.environ["SMTP_HOST"]
    port = int(os.environ.get("SMTP_PORT", "587"))
    user = os.environ["SMTP_USER"]
    passord = os.environ["SMTP_PASS"]
    fra = os.environ.get("EPOST_FRA", user)
    ...
    with smtplib.SMTP(host, port, timeout=30) as s:
        s.starttls()
        s.login(user, passord)
        s.sendmail(fra, mottakere, msg.as_string())
...
def main() -> int:
    ...
    for pos in posisjoner:
        ...
        for lok in lokaliteter:
            ...
            if hendelse:
                ...
                emne = f"⚓ {navn} har {verb} {lok['navn']} ({lok['nr']})"
                tekst = (
                    f"Fartøy:    {navn} (MMSI {mmsi})\n"
                    f"Hendelse:  {hendelse.upper()} – {lok['navn']} (lokalitet {lok['nr']})\n"
                    f"Tidspunkt: {msgtime}\n"
                    ...
                    f"– Automatisk varsel fra AIS-overvåking (BarentsWatch)"
                )
                hendelser.append((emne, tekst))
                print(f"HENDELSE: {emne}")
    ...
    for emne, tekst in hendelser:
        send_epost(emne, tekst, mottakere)
        print(f"E-post sendt: {emne}")
